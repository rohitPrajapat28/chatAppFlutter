...
class _CalendarViewState extends State<_CalendarView> {
  ...
  void _handleSelectionTypeChange() {
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      switch (selectionType) {
        ...
        case 0:
        case 5:
          if (selectionType != appData.previousSelectionType) {
            setState(() {
              widget.selectedDates.clear();
              appData.saveSelectedRange(DateTime.now(), DateTime.now());
              _showMonth(DateTime.now());
            });
          }
          break;
      }
      appData.previousSelection(selectionType);
      if (widget.selectedDates.isNotEmpty) {
        moveViewPagerToSelectedDate(
            widget.selectedDates.first ?? DateTime.now());
      }
    });
  }

  void moveViewPagerToSelectedDate(DateTime selectedDate) {
    final currentMonth = _currentMonth; // Adjust based on your state variable
    final offset = selectedDate.difference(currentMonth.day as DateTime);
    final targetPageIndex = offset.inDays ~/ DateTime.daysPerWeek;
        ...
        children: <Widget>[
                ...
                Material(
                    ...
                    child: ClipRRect(
                      ...
                      child: IconButton(
                        ...
                        onPressed:
                        _isDisplayingLastMonth ? null : handleNextMonth,
                      ),
                    )),
              ],
            ),
          ...
          Expanded(
            child: FocusableActionDetector(
              shortcuts:
              selectionType > 0 && selectionType < 5 ? null : _shortcutMap,
              actions:
              selectionType > 0 && selectionType < 5 ? null : _actionMap,
